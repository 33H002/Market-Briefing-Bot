...
2. 로컬 `config/slack_channels.yaml`의 `morning` 채널 설정을 읽어 해당 Slack 채널에 요약본을 직접 전송한다. 실제 전송에는 `channel_name`과 `channel_id`를 사용한다. Slack 메시지는 한국어로 간결하게 작성한다. 포함 내용은 날짜, 한 줄 결론, 한국/미국 핵심 이슈 3-5개, 오늘 볼 포인트, 생성된 DOCX 파일명이다. 메시지는 5000자 이하로 유지한다.
