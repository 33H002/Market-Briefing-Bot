...
한국/미국 증시 관련 주요 이슈를 리서치해 상세 `.docx` 보고서를 만들고, 사용자가 지정한 Slack 채널에 요약본을 보내기 위한 Codex automation 프로젝트입니다.

## 구성

- `src/generate_market_briefing.py`: 샘플 보고서 생성 스크립트
...
Codex automation은 매일 오전 8시 KST에 실행되도록 등록되어 있습니다. 로컬 Mac이 켜져 있고 Codex/Slack 연결이 유지되어야 합니다.
...
실행하면 현재 경로에 `market_briefing_YYYY-MM-DD.docx` 형식의 보고서가 생성됩니다.
...
실제 automation 실행 시에는 실행 당일 최신 웹 데이터를 확인하고, 접근 제한이나 지연 시세는 보고서에 명시합니다.

## Slack

요약본은 사용자가 지정한 Slack 채널로 전송합니다. 실제 채널명과 채널 ID는 공개 repo에 커밋하지 말고 로컬 automation 설정에서 관리합니다.
